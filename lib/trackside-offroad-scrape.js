...
const { Logging, OffroadClassService } = require("@vurb-tech/shared");
const { resolveOverallGroupings } = OffroadClassService;

/**
 * Flags each scraped class as an aggregate standing ("A Overall") rather than a
 * real class. Needs the whole card, since the test is whether a candidate's
 * riders also appear in a real class of the same event.
 */
function overallGroupingFlagsFor(scrapedClasses) {
  return resolveOverallGroupings(
    scrapedClasses.map((sc) => ({
      name: sc.meta.label,
      riderNames: sc.results.map((r) => r.name),
    })),
  );
}
...
const DELAY_MS = parseInt(process.env.TRACKSIDE_OFFROAD_DELAY_MS || "450", 10);
const HTTP_RETRIES = parseInt(process.env.TRACKSIDE_OFFROAD_HTTP_RETRIES || "3", 10);
const HTTP_RETRY_DELAY_MS = parseInt(
  process.env.TRACKSIDE_OFFROAD_HTTP_RETRY_DELAY_MS || "1500",
  10,
);
...
async function fetchHtml(url) {
  ...
  for (let attempt = 1; attempt <= HTTP_RETRIES; attempt++) {
    const retrySuffix =
      attempt > 1 ? ` (retry ${attempt}/${HTTP_RETRIES})` : "";

    try {
      Logging.info(`Trackside offroad GET ${url}${retrySuffix}`);
      const { status, data } = await axiosHtml.get(url, {
        responseType: "text",
        validateStatus: () => true,
      });
      ...
      lastError = new Error(`Request failed with status code ${status}`);
      if (RETRYABLE_HTTP_STATUS.has(status) && attempt < HTTP_RETRIES) {
      ...
      lastError = err;
      if (isRetryableNetworkError(err) && attempt < HTTP_RETRIES) {
        await delay(HTTP_RETRY_DELAY_MS * attempt);
        continue;
      }
      throw err;
  ...
  for (const series of seriesList) {
    ...
    for (const event of events) {
      ...
      const persistResult = await prisma.$transaction(async (tx) => {
        ...
        const groupingFlags = overallGroupingFlagsFor(classesToPersist);

        for (const [classIndex, sc] of classesToPersist.entries()) {
          const isOverallGrouping = groupingFlags[classIndex];
